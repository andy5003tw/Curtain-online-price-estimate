...
function Find-LatestInput {
  ...
  throw "No Search Performance $ModeLabel input found in $InputDir. Use the GSC Performance ZIP that contains 網頁 成效.csv/網頁.csv, or put 網頁 成效.csv directly in inbox. Coverage ZIP exports are not supported by this KPI diff."
}
...
function Normalize-Rows {
  ...
  foreach ($row in $Rows) {
    ...
    $hasQueryHeader = ($propNames | Where-Object { $_ -in @('query','Query','熱門查詢項目','查詢') }).Count -gt 0

    # Prefer explicit english headers, fallback to index positions:
    # [0]=page, [1]=clicks, [2]=impressions, [3]=ctr, [4]=position
    ...
    if ([string]::IsNullOrWhiteSpace($query)) { $query = '(all queries)' }
    ...
    # Query report may not contain page column. Allow query-only CSV by using a stable pseudo-page.
    if (-not $hasPageHeader -and $hasQueryHeader) {
      $page = '(all pages)'
    }

    # For page report, if query is missing keep stable default.
    if ([string]::IsNullOrWhiteSpace($query)) {
      $query = '(all queries)'
    }

    if ([string]::IsNullOrWhiteSpace($page) -and [string]::IsNullOrWhiteSpace($query)) { continue }
    if ([string]::IsNullOrWhiteSpace($page)) { $page = '(all pages)' }

    if (($hasPageHeader -or $page -match '^https?://') -and
  ...
  if ($validMetricRows -eq 0) {
    throw "Input does not look like a GSC Search Performance page export. Use the Performance-on-Search ZIP that contains 網頁 成效.csv/網頁.csv, or upload 網頁 成效.csv directly. Coverage ZIP exports are not supported by this KPI diff."
  }
  return $out
}
...
function Pick-PageCsvFromExtracted {
  ...
  $named = $csvFiles | Where-Object {
    Test-InputNameMatches -Name $_.Name -ExpectedNames @('網頁 成效.csv', '網頁.csv', 'page.csv', 'pages.csv')
  } | Select-Object -First 1
  if ($named) { return $named.FullName }

  foreach ($f in $csvFiles) {
    $cell = Get-FirstDataFirstCell -CsvPath $f.FullName
    if ($cell -match '^https?://') { return $f.FullName }
  }

  $named = $csvFiles | Where-Object { $_.Name -match '(?i)page|pages' } | Select-Object -First 1
  if ($named) { return $named.FullName }
...
function Normalize-Input {
  ...
  if (Test-IsZipFile -Path $InputPath) {
    ...
    $picked = Pick-PageCsvFromExtracted -ExtractDir $TempDir
...
if (-not [string]::IsNullOrWhiteSpace($InputFile)) {
  ...
  $incoming = Find-LatestInput -InputDir $inboxDir -ExpectedEntries @('網頁 成效.csv', '網頁.csv', 'page.csv', 'pages.csv') -ModeLabel 'page'
}
...
$baseline = Join-Path $historyDir 'current_baseline.normalized.csv'
if (-not (Test-Path -LiteralPath $baseline)) {
  $fallback = Join-Path $repoRoot 'scripts\gsc-data\after_28d.normalized.csv'
  if (Test-Path -LiteralPath $fallback) {
    Copy-Item -LiteralPath $fallback -Destination $baseline -Force
  } else {
...
if (Test-SameFileContent -Left $baseline -Right $afterNorm) {
  ...
  throw "This page performance input is identical to the current baseline. It was probably already run, so latest report was not overwritten. Use a newer Search Performance export, or restore an older baseline before comparing again."
}

Ensure-Dir -Path $runDir

$baselineSnapshot = Join-Path $runDir 'before_page_baseline.normalized.csv'
Copy-Item -LiteralPath $baseline -Destination $baselineSnapshot -Force

$mdReport = Join-Path $runDir 'gsc-page-kpi-diff-report(網頁).md'
$htmlReport = Join-Path $runDir 'gsc-page-kpi-diff-report(網頁).html'

Push-Location $repoRoot
try {
  & node scripts/gsc-kpi-diff-from-csv.mjs --before $baselineSnapshot --after $afterNorm --output $mdReport --title-suffix " (網頁)"
...
# Latest shortcuts
Copy-Item -LiteralPath $mdReport -Destination (Join-Path $latestDir 'gsc-page-kpi-diff-report(網頁).md') -Force
Copy-Item -LiteralPath $htmlReport -Destination (Join-Path $latestDir 'gsc-page-kpi-diff-report(網頁).html') -Force

# Run summary
$summaryPath = Join-Path $runDir 'run-summary.md'
@(
  '# Weekly SOP Page (網頁) Run Summary'
  ''
  ...
  "- Latest markdown: $(Join-Path $latestDir 'gsc-page-kpi-diff-report(網頁).md')"
  "- Latest html: $(Join-Path $latestDir 'gsc-page-kpi-diff-report(網頁).html')"
...
Write-Host 'Weekly SOP page performance workflow completed.'
Write-Host "Report folder: $runDir"
Write-Host "Latest HTML: $(Join-Path $latestDir 'gsc-page-kpi-diff-report(網頁).html')"
