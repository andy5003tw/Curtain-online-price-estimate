...
$normalizedPage = Normalize-Page -Value $Page
...
if ($matches.Count -ne 1) { throw "Expected exactly one approval-only single-page opportunity for: $Page" }
$action = $matches[0]
...
$round = [PSCustomObject][ordered]@{
  round = 1
  type = 'single-page-approved'
  priority = 1
  title = "Round 1 approved single-page review: $($action.page)"
  targetCount = 1
  targets = @([string]$action.page)
  ownerKeywords = @($action.ownerKeywords)
  action_ids = @([string]$action.action_id)
  action_fingerprints = @([string]$action.fingerprint)
...
$queue.status = 'active'
$queue.rounds = @($round)
$queue.optional_opportunities = @($queue.optional_opportunities | Where-Object { [string]$_.action_id -ne [string]$action.action_id })
foreach ($observation in @($queue.observations)) { if ((Normalize-Page -Value ([string]$observation.page)) -eq $normalizedPage) { $observation.disposition = 'queued'; $observation.reason = "使用者已核准 $([string]$action.review_type)，已建立正式單頁 executable Round。" } }

$promptRelative = 'latest/seo-geo-action-plan.round-1.slim.ai.md'
$state.status = 'active'
...
$state.rounds = @([PSCustomObject]@{ round = 1; action_ids = @([string]$action.action_id); action_fingerprints = @([string]$action.fingerprint); required_validations = $validations })
$state.prompt_files = @([PSCustomObject]@{ round = 1; path = $promptRelative; targetCount = 1; targets = @([string]$action.page); action_ids = @([string]$action.action_id); action_fingerprints = @([string]$action.fingerprint); required_validations = $validations })
$state.note = 'Single-page P0 review explicitly approved; advance requires a matching passed validation receipt.'
...
$receipt = [ordered]@{
  ...
  action_fingerprint = [string]$action.fingerprint
  round = 1
  provenance = $provenance
}

$prompt = @(
  'PLEASE IMPLEMENT THIS PLAN:', '', '# Curtain Online SEO/GEO Round 1 單頁 P0 核准執行指令', '',
  "- 使用者已核准頁面：$($action.page)", "- 核准者：$($receipt.approved_by)", "- 核准理由：$($receipt.reason)",
  "- Queue：$($queue.queue_id)", "- Cycle：$($queue.cycle_key)", '',
  '## Boundary', '', '- 只實作本頁與本 action，不新增其他頁、詞或 Round。', '- 不直接修改 out/；不得略過驗證或自行手寫 validation receipt。', '',
  '## Action', '', "- action_id：$($action.action_id)", "- page：$($action.page)", "- owner keywords：$(@($action.ownerKeywords) -join '；')", "- action type：$($action.actionType)", "- reason：$($action.reason)", '',
  '## Required Source', ''
)
foreach ($source in $requiredSource) { $prompt += ('- `{0}`' -f $source) }
$prompt += @('', '## Validation', '')
foreach ($validation in $validations) { $prompt += ('- `{0}`' -f $validation) }
$prompt += @('', '## Receipt', '', "- 驗證全數通過後，使用 scripts/write-seo-geo-validation-receipt.ps1，固定綁定 Queue $($queue.queue_id)、Cycle $($queue.cycle_key)、Round 1。")
$promptText = ($prompt -join [Environment]::NewLine) + [Environment]::NewLine
...
$files[$roundPromptPath] = $promptText
$files[$slimPromptPath] = $promptText
$files[$aiPromptPath] = $promptText
Write-FilesTransaction -Files $files

[PSCustomObject]@{ status = 'approved'; queue_id = $queue.queue_id; cycle_key = $queue.cycle_key; page = $action.page; round = 1; receipt = 'latest/seo-geo-single-page-approval.json'; prompt = $promptRelative } | ConvertTo-Json -Depth 5 -Compress
