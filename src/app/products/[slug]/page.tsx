...
const productDetails: Record<string, { features: string[]; useCases: string[]; fullDesc: string }> = {
  ...
  P002: {
    ...
    useCases: ['客廳', '餐廳', '書房'],
    fullDesc: '紗簾價格與透光不透人紗簾怎麼看最快？建議先固定同一尺寸，比較無縫紗簾效果、雙層窗簾搭配、軌道與基本安裝費，再安排台北、士林或中正區到府丈量確認正式報價。無縫紗簾適合想兼顧採光、隱私與空間通透感的人，透過整幅布料減少接縫干擾，白天能柔化採光、保留隱私，夜晚再搭配遮光布簾補足遮蔽效果，特別適合客廳、書房與落地窗場景先比較風格與預算。',
  },
  P003: {
  ...
  P007: {
    ...
    useCases: ['客廳', '書房', '餐廳'],
    fullDesc: '實木百葉窗價格試算怎麼判斷合理？先看窗戶尺寸、木種、葉片寬度、表面塗裝、窗型與安裝高度。建議先做同尺寸試算，快速比較木百葉窗簾價格、客廳落地窗木百葉方案、台北與板橋常見基本安裝費，再搭配中正區書房或景觀窗情境評估木質感與正式報價。',
  },
  P008: {
  ...
  P012: {
    ...
    fullDesc: '醫院隔簾價格通常先看隔簾尺寸、軌道長度、防焰抗菌布料、頂部網狀通風設計與施工時段。醫療隔簾專為醫院、診所、護理之家與公家醫療空間設計，布料需符合防焰與耐洗滌需求，同時兼顧病房、診療室與護理站的分區隱私。建議先用同尺寸抓醫院隔簾價格區間，再安排現場丈量確認軌道、吊掛高度與正式報價。',
  },
  P013: {
...
const productSeoExtras: Record<string, {
  ...
  P002: {
    ...
    priceTable: [
      ...
      { label: '透光不透人紗簾：落地窗 (200×240 cm)', range: '約 NT$ 2,200 – 3,800 起' },
      { label: '雙層窗簾搭配與基本安裝', range: '依軌道、窗高與遮光層確認' },
    ],
    lsiParagraph: '無縫紗簾是近年來很受歡迎的透光不透人紗簾款式，尤其適合想保留自然光、又不希望白天室內被一眼看穿的家庭。若你正在比較紗簾價格、透光不透人紗簾、紗簾推薦、雙層窗簾搭配或客廳紗簾價格，重點不只在布料本身，也包含軌道長度、垂墜高度、是否搭配遮光布簾與現場安裝條件。建議先確認想要的採光感，再做紗簾價格試算，整體報價會更接近實際需求。',
  ...
  P007: {
    ...
    lsiParagraph: '實木百葉窗價格試算是否合理，通常取決於木種、葉片寬度、表面塗裝、窗型、配件與安裝高度。台北實木百葉窗價格、板橋實木百葉窗價格與中正區常見丈量情境不一定完全相同，重點在窗型與施工條件是否一致。建議先用窗簾計算機以同尺寸試算木百葉，再查看台北、板橋與中正區地區頁的丈量流程，最後由現場確認五金規格與施工條件，讓木百葉窗簾價格與正式報價更貼近。',
    relatedBlogIds: ['blog-001'],
  },
  P008: {
    reviews: [
  ...
  P012: {
    ...
    priceTable: [
      ...
      { label: '診所隔簾、防焰抗菌布與現場安裝', range: '依軌道長度、施工時段與場域條件確認' },
    ],
    lsiParagraph: '醫院隔簾價格若要抓得準，通常要先確認隔簾寬高、軌道長度、防焰抗菌布料、頂部通風網、吊掛高度與是否需要低干擾施工。醫療隔簾、醫院窗簾、診所隔簾與護理之家隔簾都需要兼顧消防安全、清潔耐洗、分區隱私與後續維護。宏森提供台北與新北醫療空間到府丈量，先用尺寸抓醫院隔簾價格區間，再由現場確認軌道走向、布料規格與正式報價。',
    relatedBlogIds: ['blog-001'],
  },
  P013: {
    reviews: [
...
const productV3Data: Record<string, {
  ...
  P012: {
    faqs: [
      ...
      { q: '醫療隔簾需要符合哪些法規？', a: '依台灣消防法規，醫療院所的窗簾必須通過CNS 1220防燃標準測試，並取得防焰標籤認證。宏森提供的醫院隔簾均已通過此認證，可提供正式防焰測試報告，協助醫療機構通過消防稽查。' },
...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productInternalLinksMap: Record<string, Array<{ href: string; label: string }>> = {
    ...
    P002: [
      ...
      { href: '/blog/curtain-price-guide-2026/', label: '紗簾價格怎麼看：先比行情、安裝費與丈量條件' },
    ...
    P012: [
      ...
      { href: '/products/', label: '窗簾產品總覽：比較醫療隔簾、直立簾與防焰抗菌布' },
    ...
    P007: [
      ...
      { href: '/blog/curtain-price-guide-2026/', label: '2026 窗簾價格指南：實木百葉窗價格試算與基本安裝費' },
